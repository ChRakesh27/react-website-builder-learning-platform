  ...
  return (
    <>
      ...
      <div className="grid two">
        {deploymentSteps.map((step) => (
          <article className="command-card" key={step.title}>
            <h3>{step.title}</h3>
            <p>{step.text}</p>
            <CopyBox title="Command / Setting" text={step.command} />
          </article>
        ))}
      </div>
      <section className="section-block info-panel">
        ...
        <p>
          Use Vercel or Netlify first because they are simple for React
          projects. Use Firebase Hosting or Hostinger when you need a specific
          hosting setup.
        </p>
      </section>
      </div>
